<!doctype html>
<html lang="fr">
<head>
  <meta charset="utf-8">
  <title>APP TESLA</title>
  <link rel="stylesheet" href="assets/css/control.scss">
</head>
<header>
    <section class="box-controle" >
        <nav class="rotateArrow" class="one-controle">
            <a href="index.html"><img src="assets/images/symbole-fleche-droite-noir.png" height="40" width="40" alt="cerclelogo"></a>
        </nav>
    </section>
    
    <section class="center">
        <h1>Contrôles</h1>
    </section>

    <section class="cacher-tablet">
        <section class="horizontal-controle">
            <p class="p-padding-l">Ouvrir</p>
            <img  class="voiture-controle" src="assets/images/voitureteslaHaut-horizontal.png" alt="voituretesla">
            <img class="cadenasPos"  src="assets/images/cadenaslock.png" alt="cadenasLock">
            <p class="p-padding-r">Ouvrir</p>
        </section>
            <img src="assets/images/eclairlogo.png" class="eclair" alt="capotlogo">
    </section>

    <section class="cacher">
        <section class="vertical-controle">
            <p class="p-padding-h">Ouvrir</p>
            <img class="voiture-controle"  src="assets/images/voitureteslaHaut-vertical.png" alt="voituretesla">
            <img class="cadenasPos"  src="assets/images/cadenaslock.png" alt="cadenasLock">
            <p class="p-padding-b">Ouvrir</p>
        </section>
            <img src="assets/images/eclairlogo.png" class="eclair" alt="capotlogo">
    </section>
</header>
  
<body>
    <section class="bdody">
        <menu class="horizontal border1">
            
            <li>
              <button class="buttontype1" onclick="window.location.href = '';">
                <img src="assets/images/clignologo.png" height="40" width="40" alt="rondlogo">
              </button>
            </li>
                
            <li>
              <button class="buttontype1" onclick="window.location.href = '';">
                <img src="assets/images/claxonlogo.png" height="40" width="40" alt="ventilationlogo">
              </button>
            </li>
                
            <li>
              <button class="buttontype1" onclick="window.location.href = '';">
                <img src="assets/images/telecommandelogo.png" height="40" width="40" alt="eclairlogo">
              </button>
            </li>
                
            <li>
              <button class="buttontype1" onclick="window.location.href = '';">
                <img src="./assets/images/ventilate.png" height="40" width="40" alt="capotlogo">
              </button>
            </li>
                 
        </menu>
      </section>
</body>
</html>
